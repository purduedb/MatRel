...
object MatrixOperators extends Strategy {
  def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
    case SelectOperator(child, nrows, ncols, blkSize, rowOrCol, index) =>
      if (rowOrCol) {
        child match {
          case TransposeOperator(ch) =>
            MatrixTransposeExecution(planLater(
              SelectOperator(ch, ncols, nrows, blkSize, !rowOrCol, index))) :: Nil
          ...
          case MatrixScalarAddOperator(ch, alpha) =>
            MatrixScalarAddExecution(planLater(
              SelectOperator(ch, nrows, ncols, blkSize, rowOrCol, index)), alpha) :: Nil
          ...
          right, rightRowNum, rightColNum, blkSize) =>
            MatrixElementAddExecution(planLater(
              SelectOperator(left, leftRowNum, leftColNum, blkSize, rowOrCol, index)),
              leftRowNum, leftColNum, planLater(
              SelectOperator(right, rightRowNum, rightColNum, blkSize, rowOrCol, index)),
          ...
          right, rightRowNum, rightColNum, blkSize) =>
            MatrixMatrixMultiplicationExecution(planLater(
              ...
              rightRowNum, rightColNum, blkSize) :: Nil
          case _ =>
            SelectRowDirectExecution(planLater(child), blkSize, index) :: Nil
        }
      } else {
        child match {
          case TransposeOperator(ch) =>
          ...
          right, rightRowNum, rightColNum, blkSize) =>
            MatrixMatrixMultiplicationExecution(planLater(left),
              ...
              rightRowNum, rightColNum, blkSize) :: Nil
          case _ =>
            SelectColumnDirectExecution(planLater(child), blkSize, index) :: Nil
        }
      }
    case SelectCellOperator(child, nrows, ncols, blkSize, rowIdx, colIdx) =>
      child match {
        case TransposeOperator(ch) =>
          SelectCellDirectExecution(planLater(ch), blkSize, colIdx, rowIdx) :: Nil
        case MatrixScalarAddOperator(ch, alpha) =>
          MatrixScalarAddExecution(planLater(
        ...
        right, rightRowNum, rightColNum, blkSize) =>
          MatrixMatrixMultiplicationExecution(planLater(
            SelectOperator(left, leftRowNum, leftColNum, blkSize, true, rowIdx)),
            leftRowNum, leftColNum, planLater(
            SelectOperator(right, rightRowNum, rightColNum, blkSize, false, colIdx)),
            rightRowNum, rightColNum, blkSize) :: Nil
        case _ =>
          SelectCellDirectExecution(planLater(child), blkSize, rowIdx, colIdx) :: Nil
      }
